...
hardware_interface::CallbackReturn DiffDriveArduino::on_init(const hardware_interface::HardwareInfo & info)
{
  ...
  // Setting names immediately
  l_wheel_.name = cfg_.left_wheel_name;
  r_wheel_.name = cfg_.right_wheel_name;

  RCLCPP_INFO(logger_, "on_init successful");
  return hardware_interface::CallbackReturn::SUCCESS;
}

hardware_interface::CallbackReturn DiffDriveArduino::on_configure(const rclcpp_lifecycle::State & /*previous_state*/)
{
...
std::vector<hardware_interface::StateInterface> DiffDriveArduino::export_state_interfaces()
{
  ...
  {
    if (joint.name == cfg_.left_wheel_name)
    {
     state_interfaces.emplace_back(joint.name, hardware_interface::HW_IF_POSITION, &l_wheel_.pos);
    state_interfaces.emplace_back(joint.name, hardware_interface::HW_IF_VELOCITY, &l_wheel_.vel);
    }
    else if (joint.name == cfg_.right_wheel_name)
    {
     state_interfaces.emplace_back(joint.name, hardware_interface::HW_IF_POSITION, &r_wheel_.pos);
    state_interfaces.emplace_back(joint.name, hardware_interface::HW_IF_VELOCITY, &r_wheel_.vel);
    }
...
std::vector<hardware_interface::CommandInterface> DiffDriveArduino::export_command_interfaces()
{
  ...
  for (const auto & joint : info_.joints)
  {
    if (joint.name == cfg_.left_wheel_name)
    {
      command_interfaces.emplace_back(joint.name, hardware_interface::HW_IF_VELOCITY, &l_wheel_.cmd);
    }
    else if (joint.name == cfg_.right_wheel_name)
    {
      command_interfaces.emplace_back(joint.name, hardware_interface::HW_IF_VELOCITY, &r_wheel_.cmd);
    }
  }
